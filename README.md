...
1. 加入缓存组功能, 提供批量去加载, 统一释放, 同步获取接口
2. 整合所有API放到ResMgr@{单例} 
	- 初始化(初始化成功,初始化失败),
	- 检查版本(检查成功(版本信息),检查失败)
		- 版本信息 @{是否需要更新, 更新数量, 更新大小[未完成] }
	- 更新(更新成功,失败,更新中(更新信息))
		- 更新信息 @{总数量,已完成数量,总大小,已完成大小, 当前单个下载进度,当前单个下载大小,当前单个下载总大小}
	- 加载 
	- 缓存组(加载,卸载,同步获取)
3. 根据ManifestRule 生成根据Manifest 选择 package\network 不同规则来实现打包ab

- 待实现
	-[ ] 制作编辑器 只需要选择动态加载的对象,然后把不需要动态加载的自动去解析放入ab包中 ab包体大小可控制 1m-2m
	-[ ] 缓存组 做缓存机制 不要没有引用了就直接释放 而是没有引用给一定时间 之后去释放例如 无引用 60s释放
	-[ ] ManifestRule 自动去 处理 打到不同目录 而不是当前 打package/network 包在根目录 AssetBundle下
