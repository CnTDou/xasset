...
namespace Plugins.XAsset
{
    public static partial class Util
    {
        ...
        public static void GetSubPath(string folderPath, ref List<string> folder, Predicate<string> match)
        {
            DirectoryInfo dirInfo = new DirectoryInfo(folderPath);
            if (dirInfo.Exists)
            {
                foreach (var item in dirInfo.GetDirectories())
                {
                    ...
                    string path = GetUnityAssetPath(item.FullName);
                    if (match(path) && !folder.Exists((p) => { return p == path; }))
                    {
                        folder.Add(path); 
                    }
                }
